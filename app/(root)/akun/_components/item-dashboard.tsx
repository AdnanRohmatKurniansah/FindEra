...
const ItemDashboard = ({ item }: {item: itemData}) => {
  ...
  return (
    <div className="group block h-full">
      <Card className="overflow-hidden h-full gap-0 py-0 hover:shadow-md transition-shadow flex flex-col">
        <div className="relative w-full aspect-4/3 rounded-t-md overflow-hidden">
          ...
          <div className="absolute left-2 bottom-2">
            <Badge className="px-2 py-1 bg-white-transparent text-gray-600">
              ...
              {item.location_text.length > 30
                ? item.location_text.slice(0, 30) + '...'
                : item.location_text}
            </Badge>
          </div>
        </div>
        <div className="flex flex-col flex-1 px-4 py-2">
          ...
          <div className="flex items-center gap-3 pt-2 border-t mt-auto">
            <Button className="bg-blue-500 hover:bg-blue-500" asChild>
              <Link href={`/akun/edit-laporan/${item.id}`}>
                <Edit2Icon />
              </Link>
            </Button>
